...
from model import AuthzPath, AuthzPathElement
import logging
...
class BigQueryAuthzAnalyzer:
    ...
    def calc(self, fq_table_id, node: PolicyNode, path, permissions=[READ, WRITE, FULL]):
        logger.debug("calc for %s %s %s permissions = %s path = %s}", fq_table_id, node.type, node.name, permissions, list(map(lambda x: x.type, path)))
        ...
        for permission in permissions:
            for member in node.get_references(permission):
                ...
                if ref_node is None:
                    logger.error("Unable to find ref_node for member {}", member)
                    continue
    ...
    def add_to_path(self, path, node, note):
        logger.debug("Adding %s %s to path", node.type, node.name)
    ...
    def report_permission(self, fq_table_id, node, member, permission, path):
        ...
        self.add_to_path(path, node, note)
        authz = AuthzPath(fq_table_id)
        authz.set_permission(permission)
        authz.set_identity(member["principal"])
        authz.set_path(path)
        logger.info("Report: %s", authz)
        path.pop()

logger = logging.getLogger('bq-authz-analyzer')
