...
from authz_analyzer.models.model import AuthzEntry, AuthzPathElement, AuthzPathElementType
...
def export(graph: nx.DiGraph, start_node: Node, end_node: Node, writer: BaseWriter):
    for grant_path in nx.all_simple_paths(graph, start_node, end_node):
        ...
        authz_path_elements = [
            AuthzPathElement(role.name, role.name, AuthzPathElementType.ROLE, "") for role in role_path
        ]

        entry = AuthzEntry(
            asset=asset.name, permission=asset.permission_level, identity=identity.name, path=authz_path_elements
        )
