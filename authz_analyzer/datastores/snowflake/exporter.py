...
from authz_analyzer.datastores.snowflake.model import DBRole, AuthorizationModel, ResourceGrant
...
def _iter_role_row(user_name: str, role: DBRole, prev_roles: set[DBRole], roles_to_grants: Dict[str, Set[ResourceGrant]], role_to_roles: Dict[str, Set[DBRole]]) -> Generator[AuthzEntry, None, None]:
    ...
    for granted_role in role_to_roles.get(role.name, set()):
        prev_roles.add(role)
        yield from _iter_role_row(user_name=user_name, role=granted_role, prev_roles=prev_roles, roles_to_grants=roles_to_grants, role_to_roles=role_to_roles)


def export(model: AuthorizationModel, writer: BaseWriter):
    for user in model.users_to_roles.values():
        for role in user.roles:
            for entry in _iter_role_row(user.name, role=role, prev_roles=set(), roles_to_grants=model.roles_to_grants, role_to_roles=model.role_to_roles):
