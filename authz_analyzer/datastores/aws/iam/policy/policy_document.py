...
class PolicyDocument:
    ...
    def resolve(
        ...
        for statement in self.statement:
            ...
            single_stmt_service_actions_resolvers: Optional[Dict[ServiceType, ServiceActionsResolverBase]] = ActionsResolver.resolve_stmt_action_regexes(
                logger, statement.action, account_actions, allow_types_to_resolve
            )

            if single_stmt_service_actions_resolvers:
                logger.debug("Resolved actions for %s, stmt %s: %s", parent_arn, statement.sid, single_stmt_service_actions_resolvers)
                ...
                single_stmt_service_resources_resolvers: Optional[Dict[ServiceType, ServiceResourcesResolverBase]] = ResourcesResolver.resolve_stmt_resource_regexes(
                    logger, statement.resource, account_resources, allow_types_to_resolve_for_resources, single_stmt_service_actions_resolvers
                )
                ...
                if single_stmt_service_resources_resolvers:
                    for service_type, all_stmts_service_resolver in all_stmts_service_resources_resolvers.items():
                        curr_service_resolver: Optional[ServiceResourcesResolverBase] = single_stmt_service_resources_resolvers.get(service_type)
