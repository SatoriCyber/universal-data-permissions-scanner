...
class MongoDBAtlasServiceMock:
    ...
    def new(
        ...
        instance = cls(mocked_service, organization, project, cluster, connection_string, db_user, db_password, mongodb_client_mock, mongodb_database_mock, database_name)
        
        mocked_service.get_organization_info_by_id = MagicMock(side_effect=instance._side_effect_get_organization_by_id, name="get_organization")
        ...
        mocked_service.get_all_organization_users_for_project = MagicMock(side_effect=instance._side_effect_get_all_organization_users_for_project)
        mocked_service.get_teams_for_organization = MagicMock(
            side_effect=instance._side_effect_get_all_teams_for_organization, name="get_all_organization_teams_for_organization"
        )
        ...
        mongodb_client_mock.iter_database_connections = MagicMock(side_effect = instance._side_effect_iter_database_connections)
...
def generate_authz_entry(username: str, asset_name: List[str], permission: PermissionLevel, path: List[AuthzPathElement]):
...
@pytest.mark.parametrize("organization_users,organization_teams, expected_writes", (
    ( #test 1
        [OrganizationUser(id="user_id_1", email_address="user_id_1@example.com", username="user_1", teams_ids=set(), roles={"ORG_BILLING_ADMIN"})], 
        {},
        [],
    ),
    ( #test 2
        [OrganizationUser(id="user_1", email_address="user_1@example.com", username="user_1", teams_ids=set(), roles={"ORG_OWNER"})], 
        {},
        [generate_authz_entry("user_1", ["db1", "collection"], PermissionLevel.FULL, [
            AuthzPathElement("project_id", "MyProject", AuthzPathElementType.PROJECT, "cluster MyCluster is part of project MyProject"),
            AuthzPathElement("MyCluster", "MyCluster", AuthzPathElementType.CLUSTER, "database db1 is part of cluster MyCluster"),
            ])],
    ),
    ( #test 3
    [OrganizationUser(id="user_1", email_address="user_1@example.com", username="user_1", teams_ids=set(), roles={"GROUP_DATA_ACCESS_READ_ONLY"})],
    {},
    [generate_authz_entry("user_1", ["db1", "collection"], PermissionLevel.READ, [
        AuthzPathElement("MyCluster", "MyCluster", AuthzPathElementType.CLUSTER, "database db1 is part of cluster MyCluster"),
        ])
        ],
    ),
    ( #test 4
        [OrganizationUser(id="user_1", email_address="user_1@example.com", username="user_1", teams_ids={"team_id_1"}, roles=set())],
        {OrganizationTeamTestEntry(OrganizationTeam(id="team_id_1", name="team_1"), roles={"GROUP_CLUSTER_MANAGER"})},
        [],
    ),
    ( #test 5
        [OrganizationUser(id="user_1", email_address="user_1@example.com", username="user_1", teams_ids={"team_id_1"}, roles=set())],
...
def test_mongodb_atlas(organization_users: List[OrganizationUser], organization_teams: List[OrganizationTeamTestEntry], expected_writes: List[AuthzEntry]):
    ...
    mocked_service = MongoDBAtlasServiceMock.new(
        "org_id", organization_name, "project_id", project_name, "cluster_id", cluster_name, db_user, db_password, db_name, collections
    )
...
def _call_analyzer(service: MagicMock, writer: MagicMock, db_user: str, db_password: str, project_name: str, cluster_name: str):
