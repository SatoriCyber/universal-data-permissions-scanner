...
def test_user_role_no_role_resources():
    ...
    with MockCursor(grants.SINGLE_USER_ROLE, grants.NO_ROLES_GRANTS, grants.NO_ROLE_RESOURCE) as mocked_connector:
...
def test_user_role_with_grant():
    ...
    mocked_writer = MockWriter.get()
    with MockCursor(grants.SINGLE_USER_ROLE, grants.NO_ROLES_GRANTS, grants.SINGLE_GRANT_ROLE) as mocked_connector:
        _call_analyzer(mocked_connector, mocked_writer)
...
def test_user_role_to_role_grant():
    ...
    mocked_writer = MockWriter.get()
    with MockCursor(grants.SINGLE_USER_ROLE, grants.ROLE_TO_ROLE_GRANT, grants.NO_ROLE_RESOURCE) as mocked_connector:
        _call_analyzer(mocked_connector, mocked_writer)
    mocked_writer.assert_write_entry_not_called()
...
def test_user_role_to_role_with_grant():
    ...
    mocked_writer = MockWriter.get()
    with MockCursor(grants.SINGLE_USER_ROLE, grants.ROLE_TO_ROLE_GRANT, grants.GRANT_ROLE_2) as mocked_connector:
        _call_analyzer(mocked_connector, mocked_writer)
    mocked_writer.assert_write_entry_called_once_with(
        AuthzEntry(
            identity="user_1",
