...
def test_user_role_no_role_grants():
    ...
    with MockCursor(grants.USER_ONE_ROLE_ONE, grants.NO_ROLES_GRANTS) as mocked_connector:
...
def test_user_role_with_grant():
    ...
    mocked_writer = MockWriter.get()
    with MockCursor(grants.USER_ONE_ROLE_ONE, grants.ROLE_ONE_GRANT_TABLE_ONE) as mocked_connector:
        _call_analyzer(mocked_connector, mocked_writer)
...
def test_user_role_to_role_grant():
    ...
    mocked_writer = MockWriter.get()
    with MockCursor(grants.USER_ONE_ROLE_ONE, grants.ROLE_TO_ROLE_GRANT_NO_GRANTS) as mocked_connector:
        _call_analyzer(mocked_connector, mocked_writer)
    mocked_writer.assert_write_entry_not_called()
...
def test_user_role_to_role_with_grant():
    ...
    mocked_writer = MockWriter.get()
    with MockCursor(grants.USER_ONE_ROLE_ONE, grants.ROLE_ONE_TABLE_ONE_THROUGH_ROLE_TWO) as mocked_connector:
        _call_analyzer(mocked_connector, mocked_writer)
    mocked_writer.assert_write_entry_called_once_with(
        AuthzEntry(
            identity="user_1",
