...
def generate_single_dataset(access_entries: Optional[List[MockedDatasetAccessEntry]] = None) -> Dict[MockedDatasetId, MockedDataset]:
...
def generate_authz_path_element(authz_path_element_id: str, name: str, authz_path_element_type: AuthzPathElementType, note: str) -> AuthzPathElement:
...
def generate_authz_path_element_table() -> AuthzPathElement:
    return generate_authz_path_element("project1.dataset1.table1", "table1", AuthzPathElementType.TABLE, "table table1 is included in dataset dataset1_friendly_name")
...
@pytest.mark.parametrize("project_bindings", [
    [],
    [MockedBinding(
            role="NOT_RELEVANT",
            members=["user:user1"]
...
def generate_service_single_project(project_bindings: List[MockedBinding], folder: Optional[MockedFolder]=None, organization: Optional[MockedOrganization]=None, datasets: Optional[Dict[MockedDatasetId, MockedDataset]]=None):
    if datasets is None:
    ...
    service = MockBigQueryService(project=project_id, datasets=datasets, project_bindings=project_bindings, folder=folder, organization=organization)
    return service

@pytest.mark.parametrize("user_id, identity_type", [ # list of tests
...
def test_project_identity_types(user_id: str, identity_type: IdentityType):
    ...
    expected_writes = [call(AuthzEntry(
...
], ids=["roles/viewer", "roles/editor", "roles/owner", "roles/bigquery.admin", "roles/bigquery.dataEditor", 
"roles/bigquery.dataOwner", "roles/bigquery.dataViewer", "roles/bigquery.filteredDataViewer", "roles/bigquery.jobUser", 
"roles/bigquery.user", "roles/bigquerydatapolicy.maskedReader", "OWNER", "WRITER", "READER"])
...
def test_folder():
    ...
    service = generate_service_single_project(project_bindings=[], folder=MockedFolder(name="folders/folder1", bindings=folder_bindings, display_name="folder1_display_name"))
    _run_and_assert(service=service, expected_writes=expected_writes)
...
def test_org_with_folder():
    ...
    service = generate_service_single_project(project_bindings=[], folder=MockedFolder(name="folders/folder1", bindings=[], display_name="folder1_display_name", parent="organizations/1234"), organization=organization)
    _run_and_assert(service=service, expected_writes=expected_writes)
