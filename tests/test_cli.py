...
def test_snowflake():
    invoke(cli.snowflake, ['--username', 'user', '--password', 'password', '--account', 'account', '--host', 'host', '--warehouse', 'warehouse'])
...
@pytest.mark.parametrize('additional_args', [[], ['--additional-account-id', 'account2'], ['--additional-account-id', 'id123']], ids=['no-args', 'additional_account', 'external-id'])
...
@pytest.mark.parametrize('additional_args', [[], ['--port', 12345], ['--dbname', 'db123']], ids=['no-args', 'port', 'different dbname'])
...
@pytest.mark.parametrize('additional_args', [[], ['--port', 12345], ['--ssl', False]], ids=['no-args', 'port', 'disable ssl'])
...
def test_atlas():
    args = ['--public_key', 'key', '--private_key', 'private-key', '--username', 'user1', '--password', 'password', '--project', 'project1', '--cluster', 'cluster1']
